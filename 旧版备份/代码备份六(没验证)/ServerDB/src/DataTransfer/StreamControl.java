...
public class StreamControl {
	...
	private void body(){
		...
		while (true){
			...
			//获取通信一些必要信息
			if (rec[1] >= Type.BEGIN_PHONE && rec[2] <= Type.END_PHONE){
				boardOrPhoneType = Type.BEGIN_PHONE;
			}else if(rec[2] >= Type.BEGIN_BOARD && rec[2] <= Type.END_BOARD){	//获取板子通信的信息
	...
	private class EventHandleThread implements Runnable{
		...
		/*
		 * 错误码:ARM不处于监听状态，主动连接socket失败 -1,更新失败-2,异常-3
		 * 
		 */
