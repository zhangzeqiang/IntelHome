...
void * refreshInfo(){
	...
	while (1){
		if (Info.type == M_LOAD && ifLoad == LOADING){		//只有登录成功才会刷新
			...
			struct dev_node *dev_led = malloc(sizeof(struct dev_node)*4);
			if (readInfo_led(led_fd, dev_led, 4) > 0){
				for (i=0;i<4;i++){
					send[i*2+7] = dev_led[i].state;
					send[i*2+6] = dev_led[i].devno;
				}
			}else{
				for (i=0;i<4;i++){
					send[i*2+7] = 0xff;
			...
			printf("led_module complete to control\n");
			free(dev_led);
			//---------------------------------------------------
			//2、add 433 wireless refresh func
			struct dev_node dev_433;
			int result;
			int i = 0;
			int j = 0;
			while(1){
				dev_433.devno = _433_MULTICALL;			//all 433 module
				dev_433.state = _433_RETURNINFO;		//cmd to return its info
...
read_again:
				if ((result = readInfo_433(fd_433, &dev_433, 1)) <= 0){
					...
					if (i > 500){
						send[i*2+7] = 0xff;
						send[i*2+6] = _433_DEV0;
						break;
					}
					goto read_again;
					continue;
				}else{
					printf("dev_433---devno:%d\n", dev_433.devno);
					printf("dev_433---state:%d\n", dev_433.state);
					send[i*2+7] = dev_433.state;
					send[i*2+6] = dev_433.devno;
					break;
				}
			}
			//---------------------------------------------------
